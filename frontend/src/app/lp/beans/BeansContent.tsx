...
interface BeansContentProps {
  beans: BeanDetail[];
  planGroups: PlanGroup[];
}

export default function BeansContent({ beans, planGroups }: BeansContentProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const planId = searchParams.get("planId");
  const preselectedBeanId = searchParams.get("beanId");

  const validPlanIds = planGroups.flatMap((g) => [
    g.subscriptionId,
    g.singleId,
  ]);
  const plan = planGroups.find(
    (g) => g.subscriptionId === planId || g.singleId === planId,
  );
  ...
  if (!planId || (validPlanIds.length > 0 && !validPlanIds.includes(planId))) {
    redirect("/lp/catalog");
  }
